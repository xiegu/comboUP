...
server <- function(input, output, session){
  ...
  saletable <- reactive({
    ...
    ui <- list(combo1 = table1, combo2 = table2, combo3 = table3)
    return(ui)
  })
  
  observeEvent((input$sale1),{
    w$ui <- saletable()[['combo1']]
  })
  
  observeEvent(input$sale2,{
    w$ui <- saletable()[['combo2']]
  })
  
  observeEvent(input$sale3,{
    w$ui <- saletable()[['combo3']]
  })
  ...
  observeEvent(input$favor1,{
    v$ui <- favtable()[['combo1']]
  })
  
  observeEvent(input$favor2,{
    v$ui <- favtable()[['combo2']]
  })
  
  observeEvent(input$favor3,{
    v$ui <- favtable()[['combo3']]
  })
