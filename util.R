...
tableFilter <- function(cat = 'ref', preference, brand, tag, no_floor = 1, no_room = 2){
  if(cat == 'air'){
    if(no_floor ==0){
      ...
      if(isTRUE(brand)){
        haier_data_floor2 <- filter(haier_data_floor, grepl(paste(sub_brand, collapse = '|'), name))
        haier_data_wall2 <- filter(haier_data_wall, grepl(paste(sub_brand, collapse = '|'), name))
      }else{
...
comboModeler <- function(categories, tableSelector, li, tablei, room, floor, b, r){
  ...
  if(budgetChecker(sum(tablei$total), b, r)){
    ...
    for(i in categories){
      ...
      for(j in 2:n){
        ...
        table1 <- mutate(table1, category = categories, total = ifelse(category=='floor', floor*price, ifelse(category == 'wall', (room - floor)*price, price)))
    ...
    if(budgetChecker(sum(table1$total), b, r)){
      ...
      for(i in categories){
        ...
        if(indexi>1){
          price2ndMax <- sort(tableSelector[[indexi-1]]$price, decreasing = TRUE)[2]
          indexP2ndMax <- which(tableSelector[[indexi-1]]$price == price2ndMax)[1]
        ...
        for(j in 1:n){
          ...
          table2 <- mutate(table2, category = categories, total = ifelse(category=='floor', floor*price, ifelse(category == 'wall', (room - floor)*price, price)))
...
comboModelerNext <- function(cat, tabSel, ls, ro, f, b, r){
  ...
  tablei <-  mutate(tablei, category = cat, total = ifelse(category=='floor', f*price, ifelse(category == 'wall', (ro - f)*price, price)))
  if(budgetChecker(sum(tablei$total), b, r)){
    table <- tablei
    l <- li
    tabSel <- tabSel
    ...
    for(i in cat){
      ...
      for(j in 2:n){
        ...
        l1[[i]] <- product
        table1 <- do.call(rbind, l1)
        table1 <- mutate(table1, category = cat, total = ifelse(category=='floor', f*price, ifelse(category == 'wall', (ro - f)*price, price)))
        if(budgetChecker(sum(table1$total), b, r)){
          break
        }else{next}
      }
    ...
    if(budgetChecker(sum(table1$total), b, r)){
      ...
      for(i in cat){
        ...
        if(indexi>1){
          ...
          indexP2ndMax <- which(tabSel[[indexi-1]]$price == price2ndMax)[1]
        ...
        for(j in 1:n){
          ...
          l2[[i]] <- product
          table2 <- do.call(rbind, l2)
          table2 <- mutate(table2, category = cat, total = ifelse(category=='floor', f*price, ifelse(category == 'wall', (ro - f)*price, price)))
          if(budgetChecker(sum(table2$total), b, r)){
            break
          }else{next}
        }
  ...
  return(list(l = l, table = table, tableSelector = tabSel))
}